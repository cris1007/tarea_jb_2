...
public class Steps {
	...
	@Cuando("se hace una petición {string} con un sku existente")
	public void se_hace_una_petición_con_un_sku_existente(String metodo) {
		currentMethod=metodo;
	}
	
	@Cuando("se hace una petición {string}")
	public void se_hace_una_petición(String metodo) {
		currentMethod=metodo;
	}
	
	@Cuando("se hace una petición {string} con un sku no existente")
	public void se_hace_una_petición_con_un_sku_no_existente(String metodo) {
		currentMethod=metodo;
		originalSku="/api/v1/product/"+"aaaaaaaa-bbbbb";
	}
		...
		if ("POST".equals(currentMethod.toUpperCase())) {
			...
			response = 	given()
					.log().all()
							...
							//.body(GlobalHook.productRequest)
						.when()
							.delete(currentUrl + originalSku + "/")// Uri
						.then()
							.log().all()
							.extract().response();
			...
			response = 	given()
					.log().all()
							...
							//.body(GlobalHook.productRequest)
						.when()
							.get(currentUrl+ "/api/v1/product/")// Uri
						.then()
							.log().all()
							.extract().response();
		}
